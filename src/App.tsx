...
export default function App() {
  ...
  return <div className='shell app-shell'><div className='app'><main className='main page-content'>
    {tab === 'home' && <section className='stack page'>
      ...
      <section><div className='secHead'><h3>今日灵感</h3></div><div className='hScroll horizontal-scroll'>{ideas.map((i, idx) => {
    ...
    {tab === 'meal' && <section className='stack page'><div className='titleCard'><h2>安排下一餐</h2><p>选一道想吃的菜，约好属于我们的时间。</p></div>
      <div className='hScroll horizontal-scroll'>{['今天', '明天', '周末', '自定义日期'].map((d) => <button key={d} className='dateCard'>{d}</button>)}</div>
    ...
    {tab === 'menu' && <section className='stack page'><div className='titleCard'><h2>私房菜单</h2><p>这些是小白和小鸡毛常吃、想吃、值得再做的菜。</p></div><div className='searchBox'><Search size={16}/><input placeholder='搜索今天想吃的菜' value={search} onChange={(e) => setSearch(e.target.value)} /></div><div className='chips'>{cuisines.map((c) => <button key={c} className={cuisine === c ? 'activeChip' : ''} onClick={() => setCuisine(c)}>{c}</button>)}</div>
