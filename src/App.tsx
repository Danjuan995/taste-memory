...
export default function App() {
  ...
  const [recipeViewMode, setRecipeViewMode] = useState<'list' | 'create' | 'detail' | 'edit'>('list');
  ...
  return <div className='shell app-shell'><div className='app'><main className='main page-content'>
    ...
    {tab === 'recipes' && <section className='stack page'>
      ...
      <div className='chips'>{cuisines.map((c) => <button key={c} className={cuisine === c ? 'activeChip' : ''} onClick={() => setCuisine(c)}>{c}</button>)}</div>
      <button className='pillBtn' onClick={() => { setEditingRecipeId(''); setSelectedRecipeId(''); setPrefillRecipeDishId(undefined); setRecipeViewMode('create'); }}><Plus size={16}/> 新增菜谱</button>
      {store.recipes.filter((r) => r.name.includes(search) && (cuisine === '全部' || r.cuisine === cuisine)).length === 0 ? <div className='empty'>还没有菜谱记录，把第一道私房菜的做法记下来吧。<button className='tinyBtn' onClick={() => { setEditingRecipeId(''); setSelectedRecipeId(''); setRecipeViewMode('create'); }}>新增菜谱</button></div> :
      <div className='grid1'>{store.recipes.filter((r) => r.name.includes(search) && (cuisine === '全部' || r.cuisine === cuisine)).map((r) => <article key={r.id} className='albumCard recipeRecordCard'><img src={r.coverImageUrl || ''} alt={r.name} /><div><h4>{r.name}</h4><small>关联菜品：{store.dishes.find((d) => d.id === r.dishId)?.name || '未关联'} · {r.cuisine}</small><small>{difficultyLabel(r.difficulty)} · {r.cookingTime}分钟 · 食材{r.ingredients.length} · 步骤{r.steps.length}</small><small>创建人：{r.createdBy === 'xiaobai' ? '小白' : '小鸡毛'} · 最近更新：{r.updatedAt.slice(0, 10)}</small><div className='tags'>{r.tags.map((t) => <span key={t}>{t}</span>)}</div><div className='cardActions'><button className='tinyBtn' onClick={() => { setSelectedRecipeId(r.id); setRecipeViewMode('detail'); }}>查看</button><button className='tinyBtn secondaryBtn' onClick={() => { setEditingRecipeId(r.id); setSelectedRecipeId(''); setRecipeViewMode('edit'); }}>编辑</button><button className='tinyBtn' onClick={() => { setTab('meal'); setOverlay('order'); }}>加入点菜</button><button className='tinyBtn dangerBtn' onClick={() => persist(storageService.deleteRecipe(store, r.id))}>删除</button></div></div></article>)}</div>}
      </>}
      {(recipeViewMode === 'create' || recipeViewMode === 'edit') && <RecipeFormView
        recipe={store.recipes.find((r) => r.id === editingRecipeId)}
        dishes={store.dishes}
        identity={identity}
        prefillDishId={prefillRecipeDishId}
        isEdit={recipeViewMode === 'edit'}
        onCancel={() => setRecipeViewMode('list')}
        onSave={(v) => {
          persist(editingRecipeId ? storageService.updateRecipe(store, editingRecipeId, v) : storageService.addRecipe(store, v));
          setEditingRecipeId('');
          setSelectedRecipeId('');
          setPrefillRecipeDishId(undefined);
          setRecipeViewMode('list');
        }}
      />}
      {recipeViewMode === 'detail' && selectedRecipe && <RecipeDetailView recipe={selectedRecipe} dish={store.dishes.find((d) => d.id === selectedRecipe.dishId)} onBack={() => setRecipeViewMode('list')} onEdit={() => { setEditingRecipeId(selectedRecipe.id); setRecipeViewMode('edit'); }} onAddMeal={() => { setTab('meal'); setOverlay('order'); }} />}
    </section>}
...
function RecipeFormView({ recipe, dishes, identity, prefillDishId, isEdit, onSave, onCancel }: { recipe?: Recipe; dishes: Dish[]; identity: UserName; prefillDishId?: string; isEdit: boolean; onSave: (v: Omit<Recipe, 'id' | 'createdAt' | 'updatedAt'>) => void; onCancel: () => void }) {
  const [error, setError] = useState('');
  const [v, setV] = useState<Omit<Recipe, 'id' | 'createdAt' | 'updatedAt'>>(recipe ? { ...recipe } : { name: '', coverImageUrl: '', dishId: prefillDishId, cuisine: '家常菜', description: '', ingredients: [{ id: crypto.randomUUID(), name: '', amount: '', note: '' }], steps: [{ id: crypto.randomUUID(), order: 1, title: '', description: '', imageUrl: '', duration: undefined }], tips: '', difficulty: 'normal', cookingTime: 30, servings: 2, createdBy: identity === '小白' ? 'xiaobai' : 'xiaojimao', tags: [] });
  const sectionTitle = isEdit ? '编辑菜谱' : '新增菜谱';
  const submit = (e: FormEvent) => {
    e.preventDefault();
    const ingredients = v.ingredients.filter((i) => i.name.trim() || i.amount?.trim() || i.note?.trim()).map((i) => ({ ...i, name: i.name.trim() }));
    const steps = v.steps.filter((s) => s.description.trim()).map((s, idx) => ({ ...s, order: idx + 1, description: s.description.trim() }));
    if (!v.name.trim()) return setError('请填写菜谱名称。');
    if (ingredients.filter((i) => i.name).length === 0) return setError('请至少填写 1 条有效食材。');
    if (steps.length === 0) return setError('请至少填写 1 条有效步骤说明。');
    setError('');
    onSave({ ...v, name: v.name.trim(), dishId: v.dishId || undefined, ingredients: ingredients.filter((i) => i.name), steps, tags: String(v.tags).split(/[，,\s]+/).map((t) => t.trim()).filter(Boolean), cookingTime: Number(v.cookingTime) || 0, servings: v.servings ? Number(v.servings) : undefined });
  };
  return <section className='stack page'><div className='titleCard'><button className='tinyBtn secondaryBtn' onClick={onCancel}>返回菜谱</button><h2>{sectionTitle}</h2><p>把这道菜的食材、步骤和小技巧认真记下来。</p></div><form className='form' onSubmit={submit}><div className='groupCard'><h4>基础信息</h4>{field({ placeholder: '例如 番茄牛腩', value: v.name, onChange: (e) => setV({ ...v, name: e.currentTarget.value }) })}{field({ placeholder: '粘贴一张菜品图片链接', value: v.coverImageUrl || '', onChange: (e) => setV({ ...v, coverImageUrl: e.currentTarget.value }), required: false })}<select className='niceInput' value={v.dishId || ''} onChange={(e) => setV({ ...v, dishId: e.currentTarget.value || undefined })}><option value=''>关联菜品（可选）</option>{dishes.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</select><select className='niceInput' value={v.cuisine} onChange={(e) => setV({ ...v, cuisine: e.currentTarget.value as CuisineType })}>{cuisines.slice(1).map((c) => <option key={c}>{c}</option>)}</select><textarea className='niceInput' placeholder='简单记录这道菜的特点' value={v.description || ''} onChange={(e) => setV({ ...v, description: e.currentTarget.value })} />{field({ type: 'number', placeholder: '制作时间（分钟）', value: v.cookingTime, onChange: (e) => setV({ ...v, cookingTime: Number(e.currentTarget.value) }) })}<select className='niceInput' value={v.difficulty} onChange={(e) => setV({ ...v, difficulty: e.currentTarget.value as RecipeDifficulty })}><option value='easy'>简单</option><option value='normal'>普通</option><option value='hard'>稍难</option></select>{field({ type: 'number', placeholder: '例如 2', value: v.servings || '', required: false, onChange: (e) => setV({ ...v, servings: Number(e.currentTarget.value) }) })}<select className='niceInput' value={v.createdBy} onChange={(e) => setV({ ...v, createdBy: e.currentTarget.value as 'xiaobai' | 'xiaojimao' })}><option value='xiaobai'>小白</option><option value='xiaojimao'>小鸡毛</option></select>{field({ placeholder: '例如 暖胃、家常、浓郁', value: String(v.tags), required: false, onChange: (e) => setV({ ...v, tags: e.currentTarget.value.split(/[，,\s]+/).filter(Boolean) }) })}</div>
  <div className='groupCard'><h4>食材清单</h4>{v.ingredients.map((ing, idx) => <div key={ing.id} className='groupCard'>{field({ placeholder: `食材名称（如 牛腩）`, value: ing.name, required: idx === 0, onChange: (e) => setV({ ...v, ingredients: v.ingredients.map((x) => x.id === ing.id ? { ...x, name: e.currentTarget.value } : x) }) })}{field({ placeholder: '用量（如 500g）', value: ing.amount || '', required: false, onChange: (e) => setV({ ...v, ingredients: v.ingredients.map((x) => x.id === ing.id ? { ...x, amount: e.currentTarget.value } : x) }) })}{field({ placeholder: '可选，例如 切块', value: ing.note || '', required: false, onChange: (e) => setV({ ...v, ingredients: v.ingredients.map((x) => x.id === ing.id ? { ...x, note: e.currentTarget.value } : x) }) })}<button type='button' className='tinyBtn dangerBtn' onClick={() => setV({ ...v, ingredients: v.ingredients.length > 1 ? v.ingredients.filter((x) => x.id !== ing.id) : v.ingredients })}>删除食材</button></div>)}<button type='button' className='tinyBtn' onClick={() => setV({ ...v, ingredients: [...v.ingredients, { id: crypto.randomUUID(), name: '', amount: '', note: '' }] })}>添加食材</button></div>
  <div className='groupCard'><h4>做法步骤</h4>{v.steps.map((s, idx) => <div key={s.id} className='groupCard'><p>步骤 {idx + 1}</p>{field({ placeholder: '步骤标题，可选（如 焯水）', required: false, value: s.title || '', onChange: (e) => setV({ ...v, steps: v.steps.map((x) => x.id === s.id ? { ...x, title: e.currentTarget.value } : x) }) })}<textarea className='niceInput' placeholder='步骤说明' value={s.description} onChange={(e) => setV({ ...v, steps: v.steps.map((x) => x.id === s.id ? { ...x, description: e.currentTarget.value } : x) })} />{field({ placeholder: '步骤图片 URL，可选', required: false, value: s.imageUrl || '', onChange: (e) => setV({ ...v, steps: v.steps.map((x) => x.id === s.id ? { ...x, imageUrl: e.currentTarget.value } : x) }) })}{field({ type: 'number', placeholder: '预计时长（分钟）', required: false, value: s.duration || '', onChange: (e) => setV({ ...v, steps: v.steps.map((x) => x.id === s.id ? { ...x, duration: Number(e.currentTarget.value) } : x) }) })}<button type='button' className='tinyBtn dangerBtn' onClick={() => setV({ ...v, steps: v.steps.length > 1 ? v.steps.filter((x) => x.id !== s.id).map((x, i) => ({ ...x, order: i + 1 })) : v.steps })}>删除步骤</button></div>)}<button type='button' className='tinyBtn' onClick={() => setV({ ...v, steps: [...v.steps, { id: crypto.randomUUID(), order: v.steps.length + 1, description: '' }] })}>添加步骤</button></div>
  <div className='groupCard'><h4>小贴士</h4><textarea className='niceInput' placeholder='例如：番茄可以一半炒出沙，一半后放保留口感。牛腩提前炖软会更入味。' value={v.tips || ''} onChange={(e) => setV({ ...v, tips: e.currentTarget.value })} /></div>
  {error ? <p className='errorText'>{error}</p> : null}
  <div className='cardActions'><button type='button' className='tinyBtn secondaryBtn' onClick={onCancel}>取消</button><button className='pillBtn'>保存菜谱</button></div></form></section>;
}

function RecipeDetailView({ recipe, dish, onEdit, onAddMeal, onBack }: { recipe: Recipe; dish?: Dish; onEdit: () => void; onAddMeal: () => void; onBack: () => void }) {
  return <article className='recipeDetail card'><button className='tinyBtn secondaryBtn' onClick={onBack}>返回菜谱</button><img className='detailCover' src={recipe.coverImageUrl || ''} alt={recipe.name} /><h3>{recipe.name}</h3><p>{recipe.description}</p><div className='tags'><span>{recipe.cuisine}</span><span>{difficultyLabel(recipe.difficulty)}</span><span>{recipe.cookingTime} 分钟</span>{recipe.servings ? <span>{recipe.servings} 人份</span> : null}</div><p>关联菜品：{dish?.name || '未关联'}</p><h4>食材清单</h4><div className='ingredients'>{recipe.ingredients.map((i) => <div key={i.id} className='ingredientItem'><b>{i.name}</b><small>{i.amount || '适量'} {i.note ? `· ${i.note}` : ''}</small></div>)}</div><h4>做法步骤</h4><div className='steps'>{[...recipe.steps].sort((a, b) => a.order - b.order).map((s) => <div key={s.id} className='stepItem'><div className='stepNo'>{s.order}</div><div><p>{s.title || `步骤 ${s.order}`}</p><small>{s.description}</small>{s.duration ? <em>{s.duration} 分钟</em> : null}</div></div>)}</div>{recipe.tips ? <><h4>小贴士</h4><p className='aiSum'>{recipe.tips}</p></> : null}<small>创建人：{recipe.createdBy === 'xiaobai' ? '小白' : '小鸡毛'} · 更新于 {recipe.updatedAt.slice(0, 10)}</small><div className='cardActions'><button className='tinyBtn secondaryBtn' onClick={onEdit}>编辑菜谱</button><button className='tinyBtn' onClick={onAddMeal}>加入点菜</button></div></article>;
}
