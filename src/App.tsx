...
function RecipeFormView({ recipe, dishes, identity, prefillDishId, isEdit, onSave, onCancel }: { recipe?: Recipe; dishes: Dish[]; identity: UserName; prefillDishId?: string; isEdit: boolean; onSave: (v: Omit<Recipe, 'id' | 'createdAt' | 'updatedAt'>) => void; onCancel: () => void }) {
  ...
  const [v, setV] = useState<Omit<Recipe, 'id' | 'createdAt' | 'updatedAt'>>(recipe ? { ...recipe } : { name: '', coverImageUrl: '', dishId: prefillDishId, cuisine: '家常菜', description: '', ingredients: [{ id: createId('ingredient'), name: '', amount: '', note: '' }], steps: [{ id: createId('step'), order: 1, title: '', description: '', imageUrl: '', duration: undefined }], tips: '', difficulty: 'normal', cookingTime: 30, servings: 2, createdBy: identity === '小白' ? 'xiaobai' : 'xiaojimao', tags: [] });
