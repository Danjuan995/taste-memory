...
export interface StoreData { dishes: Dish[]; orders: MealOrder[]; memories: TasteMemory[]; restaurants: Restaurant[]; recipes: Recipe[]; }
...
const normalizeMemory = (m: TasteMemory): TasteMemory => ({ ...m, imageUrl: m.imageUrl || '', review: m.review || m.comment || '', experience: m.experience || m.review || m.aiSummary || '', message: m.message || m.note || '', note: m.note || m.message || '', tags: m.tags || m.aiTags || [], aiTags: m.aiTags || m.tags || [], rating: typeof m.rating === 'number' ? m.rating : undefined });
...
export const storageService = {
  loadAll(): StoreData {
    ...
    try {
      ...
      return { ...fallback, ...parsed, recipes: parsed.recipes ?? [], memories: (parsed.memories ?? fallback.memories).map(normalizeMemory) };
  ...
  addMemory(data: StoreData, memory: Omit<TasteMemory, 'id' | 'createdAt'>) { return { ...data, memories: [normalizeMemory(withId<TasteMemory>({ ...memory, createdAt: new Date().toISOString() } as Omit<TasteMemory, 'id'>)), ...data.memories] }; },
  ...
  addRecipe(data: StoreData, recipe: Omit<Recipe, 'id' | 'createdAt' | 'updatedAt'>) { const now = new Date().toISOString(); return { ...data, recipes: [{ ...withId<Recipe>({ ...recipe, createdAt: now, updatedAt: now } as Omit<Recipe, 'id'>) }, ...data.recipes] }; },
